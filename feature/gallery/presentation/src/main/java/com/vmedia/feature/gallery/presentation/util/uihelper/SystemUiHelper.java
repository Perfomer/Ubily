package com.vmedia.feature.gallery.presentation.util.uihelper;
 ...
 * the required level. The level specifies the extent to which the System UI's visibility is changed when you call {@link #hide()} or {@link
 * #toggle()}.
 */
public final class SystemUiHelper {
   ...
   * <p>
   * <p>The {@link #FLAG_IMMERSIVE_STICKY} flag can be used to control how the system bars are displayed.
   */
  public static final int LEVEL_IMMERSIVE = 3;

  /**
   * When this flag is set, the {@link android.view.WindowManager.LayoutParams#FLAG_LAYOUT_IN_SCREEN} flag will be set on older devices, making the
   * status bar "float" on top of the activity layout. This is most useful when there are no controls at the top of the activity layout. <p> This
   * flag isn't used on newer devices because the <a href="http://developer.android.com/design/patterns/actionbar.html">action bar</a>, the most
   * important structural element of an Android app, should be visible and not obscured by the system UI.
   */
  public static final int FLAG_LAYOUT_IN_SCREEN_OLDER_DEVICES = 0x1;

  /**
   * Used with {@link #LEVEL_IMMERSIVE}. When this flag is set, an inward swipe in the system bars areas will cause the system bars to temporarily
   ...
   * @param level    The level of hiding. Should be either {@link #LEVEL_LOW_PROFILE}, {@link #LEVEL_HIDE_STATUS_BAR}, {@link #LEVEL_LEAN_BACK} or
   *                 {@link #LEVEL_IMMERSIVE}
   * @param flags    Additional options. See {@link #FLAG_LAYOUT_IN_SCREEN_OLDER_DEVICES} and {@link #FLAG_IMMERSIVE_STICKY}
   * @param listener A listener which is called when the system visibility is changed
   */
  public SystemUiHelper(Activity activity, int level, int flags, @Nullable OnSystemUiVisibilityChangeListener listener) {
  ...
  /**
   * @return true if the system UI is currently showing. What this means depends on the mode this {@link SystemUiHelper} was instantiated with.
   */
  ...
  /**
   * Show the system UI. What this means depends on the mode this {@link SystemUiHelper} was instantiated with.
   * <p>
  ...
  /**
   * Hide the system UI. What this means depends on the mode this {@link SystemUiHelper} was instantiated with.
   * <p>
   * <p>Any currently queued delayed hide requests will be removed.
   */
  public void hide() {
  ...
  static abstract class SystemUiHelperImpl {
    ...
    SystemUiHelperImpl(Activity activity, int level, int flags, OnSystemUiVisibilityChangeListener onSystemUiVisibilityChangeListener) {
      mActivity = activity;
      mLevel = level;
      mFlags = flags;
      mOnSystemUiVisibilityChangeListener = onSystemUiVisibilityChangeListener;
    }
