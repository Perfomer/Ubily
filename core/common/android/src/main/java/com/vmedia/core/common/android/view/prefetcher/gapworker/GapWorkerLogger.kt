package com.vmedia.core.common.android.view.prefetcher.gapworker

import android.os.AsyncTask
import android.view.View
import androidx.recyclerview.widget.RecyclerView
import com.vmedia.core.common.android.view.prefetcher.gapworker.LogEvent.*
...
internal sealed class LogEvent {
    ...
    internal data class Task(
        val position: Int,
        val distanceToItem: Int,
        val viewVelocity: Int,
        val immediate: Boolean
    ) : LogEvent()

    internal data class Attached(
        val view: RecyclerView,
        val position: Int
    ) : LogEvent()

    internal data class Deadline(
        val remainingTime: Long?,
        val exiting: Boolean
    ) : LogEvent() {

        override fun toString(): String {
            val remainingTime = String.format("%,d", remainingTime)
            return "Deadline(remainingTime=$remainingTime, exiting=$exiting)"
        }
    ...
    internal class Attempt(
        ...
        override fun toString(): String {
            return "Attempt(view=${view.javaClass.simpleName}, item=${item?.javaClass?.simpleName}, position=$position, bound=$bound)"
        }
    ...
    inline fun dump(fromIdleHandler: Boolean) {
        if (LOGGING_ENABLED) {
            ...
            AsyncTask.SERIAL_EXECUTOR.execute {
                Timber.tag("WrappedGapWorker")
                    .d("Go from ${"IdleHandler".takeIf { fromIdleHandler } ?: "RecyclerView"}:")
