...
internal class ProvisionMapperTest {
    ...
    private val mapper: ProvisionMapper<EventSale>
        get() = SaleMapper(source)

    @Test
    fun map_empty() {
        // Given
        whenever(source.getEventSales(any())).thenReturn(Single.just(emptyList()))
    ...
    fun map() {
        // Given
        whenever(source.getEventSales(any())).thenReturn(Single.just(MOCKY_SALES))
